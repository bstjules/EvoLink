...
public class JSONimporter : MonoBehaviour
{
    ...
    public void ImportNodes()
    {
        ...
        {
            GameObject.DestroyImmediate(NodeBucket.transform.GetChild(i).gameObject);
        }
        NodeBucket.transform.DetachChildren();

        for (int i = TileBucket.transform.childCount - 1; i >= 0; --i)
        {
            GameObject.DestroyImmediate(TileBucket.transform.GetChild(i).gameObject);
        }
        ...
        {
            var placeposition = new Vector3(i * 150 - 390, -715, 1);
